...
- All real logic lives in TypeScript under `src/compat/` and is bundled into
  `assets/skills/comet/scripts/comet-runtime.mjs` by
  `scripts/build-classic-runtime.mjs` (esbuild). **Run `pnpm build` after editing
  anything in `src/compat/`**, or the tests will exercise a stale bundle and the
...
- When adding or renaming a launcher, update `assets/manifest.json` (`skills[]`
  ...
  in `test/ts/comet-scripts.test.ts`, and keep `.codex/skills/comet/scripts/`
...
comet-runtime.mjs  <-  every comet-*.mjs launcher imports it
  └─ src/compat/classic-cli.ts dispatches: state / validate / guard / handoff / archive / hook-guard
...
1. `src/compat/classic-state-command.ts` for the `set` whitelist and enum
   validation (`SETTABLE_FIELDS` / `MACHINE_OWNED_FIELDS`).
2. `src/compat/classic-validate-command.ts` for schema validation and the known
   field set.
