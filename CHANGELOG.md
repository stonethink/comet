...
This is the first beta of the 0.4.0 line. Relative to 0.3.9, Comet becomes a Node-only runtime and gains three headline capabilities: orchestrate **any** Skills into custom workflows with `/comet-any`, evaluate **any** local Skill with `comet eval`, and visualize every change in a browser dashboard with `comet dashboard`. The sections below cover only what a 0.3.9 user would notice.

### Added

- **Node-only runtime**: Every bundled Comet workflow script now runs through thin `.mjs` launchers backed by a shared TypeScript runtime. The same `/comet` workflow now works on Windows, macOS, and Linux without Git Bash, WSL, or shell-script compatibility. Classic `.comet.yaml` state is still understood via a typed state projection, so existing changes keep working.
- **`comet dashboard` command** (new): Boots a local, read-only browser dashboard that shows every active and archived change with phase progress (Open → Design → Build → Verify → Archive), artifact checklist, task breakdown, verify status, next-action guidance, and a Git snapshot. `--json` prints one snapshot for CI/scripts, `--port` pins a port, `--no-open` skips the browser for SSH/containers, and `GET /api/dashboard` exposes the same snapshot to scripts.
- **`comet skill` commands** (new): Adds `comet skill add|show|run|continue|check` for local Skill packages — install a Skill into a project, inspect it, run it with immutable snapshots, resume pending actions, and run deterministic checks, so Skill execution is auditable instead of purely conversational.
- **`comet eval` harness** (new): Adds `comet eval [target]` to evaluate Comet workflows or any local Skill with generated manifests, task profiles, HTML reports, token/cost attribution, and reusable regression checks. Repeated runs yield pass@k / pass^k capability-vs-reliability metrics.
- **`/comet-any` Skill Creator** (new): `/comet-any` is now the main path for creating or upgrading reusable Skills. It guides you through candidate discovery, a confirmable proposal, generated Skill bundles, readiness feedback, eval planning, review approval, and publish/distribution. A proposal can declare Workflow Nodes, Skill Bindings, Output Schemas, Guardrails, Handoffs, and Required Skill Calls, so a custom multi-step workflow behaves like a real composed Skill instead of a single thin wrapper.
- **`comet creator` command** (new): Adds `comet creator list|status|next|guide|candidates|propose|init|resolve|authoring-plan|authoring-record|generate` as the user-facing CLI surface for `/comet-any` creation, resume, and authoring flows without exposing backend Bundle factory command names.
- **`comet publish` command** (new): The release path for `/comet-any` outputs after Skill Creator and eval evidence are ready. It requires review approval, runs the publish step, and previews distribution before writing files to target platforms.
- **Skill preferences** (new): Adds `.comet/skill-preferences.yaml` with inventory-backed setup, proposal previews, and readiness feedback, so you can guide `/comet-any` Skill creation and preferred ordering without editing internal Bundle files.
...
- **Antigravity 2.0 platform support**: Adds native support for Antigravity 2.0 / Antigravity IDE 2.0. Global skills install under `~/.gemini/config/skills/`, while existing Antigravity (`~/.gemini/antigravity/skills/`) and Gemini (`~/.gemini/skills/`) clients remain compatible.
...
- **Classic command scripts**: Splits the shipped Classic automation out of the single `comet-runtime.mjs` dispatcher into independent Node command bundles (`comet-state.mjs`, `comet-guard.mjs`, `comet-archive.mjs`, and peers), so each script owns its own runtime entry while shared behavior still comes from the TypeScript Classic domain modules.
- **No double review under `subagent-driven-development`**: The subagent dispatch reference now states explicitly that `review_mode` takes over the Superpowers `subagent-driven-development` default of dispatching a task reviewer after every task, and that the per-mode review count is fixed by a whole-flow review-budget table — so a change no longer pays for both the Superpowers per-task review loop AND Comet's own reviews. The CRITICAL override block now names review_mode as the single authority on which tasks get a task-level reviewer, preventing agents from re-adding reviewers the budget removed.
- **Superpowers `subagent-driven-development` contract alignment**: The Comet subagent dispatch extension now describes review in terms of the public task reviewer contract instead of the older spec-reviewer/code-quality-reviewer split. It also preserves Superpowers v6 `subagent-driven-development` public flow semantics — pre-flight plan review, file-based handoffs for bulky task/review material, neutral reviewer prompts, reviewer "cannot verify from material alone" handling, and progress-record reconciliation — while stating that Comet does not read, write, or require Superpowers `subagent-driven-development` internal scripts or workspace paths. `review_mode` remains Comet's review-trigger policy; the installed Superpowers `subagent-driven-development` skill owns its own implementation details.
- **Three-mode review gradient**: `review_mode` now forms a true gradient instead of three shades of "batched" review. `off` = zero task-level reviewers (user accepts risk, reason recorded). `standard` = a task-level reviewer (spec + quality) only when a task hits a risk signal — driven by both the implementer's self-reported risk signals and the coordinator's diff review (cross-module change, security-sensitive surface, concurrency/locks, data/schema migration, public API change, `DONE_WITH_CONCERNS`, or >200-line diff); low-risk tasks skip straight to checkoff, keeping standard fast. `thorough` = a task-level reviewer on every task (spec + quality), replacing the previous batched review: a high-risk change demands immediate, focused review per task, and deferring to a batch boundary to catch issues was too costly. The progress checkpoint records review-fix rounds per task and whether a standard risk-review already fired, so recovery does not re-dispatch completed reviews. The `comet-build` recommendation table and per-task acceptance line were updated to match (standard = risk-triggered, thorough = every task), removing the stale batched-review descriptions.
- **`executing-plans` review gate scaled by review_mode**: Previously `executing-plans` ran a single final code review for both `standard` and `thorough`, so `thorough` added no review value under that build mode. The gate now branches by mode: `off` skips review (reason recorded), `standard` does one lightweight whole-change review, and `thorough` adds a segmented review every 3 tasks (scoped to each segment's diff) on top of the final review — the closest per-task-equivalent available when there is no isolated implementer subagent to review per task. This makes the off < standard < thorough gradient hold under both build modes, even though `executing-plans` cannot match `subagent-driven-development`'s literal per-task reviewer.
- **Mandatory model selection per dispatch**: The subagent dispatch reference now requires every implementer, fixer, and reviewer dispatch to specify the model explicitly (following the Superpowers Model Selection rules), instead of silently inheriting the session's most expensive model. This is a direct speedup for the slow subagent execution path: mechanical implementers drop to the cheapest tier, reviewers scale to diff risk, and only the final whole-branch review uses the most capable model.
- **Product positioning**: Comet is now positioned as a resumable workflow and Skill platform rather than an OpenSpec + Superpowers script bundle. README, CLI help, and `/comet-any` guidance lead with what you do: run workflows, create Skills, evaluate them, publish them, and diagnose stuck changes.
- **Run state storage**: Machine-owned run state moved out of `.comet.yaml` into `.comet/run-state.json`, keeping user-editable workflow fields separate from engine checkpoints and reducing accidental state corruption.
- **Classic state transition audit log**: Classic phase transitions now share one TypeScript transition table and append successful `comet-state transition`, `comet-guard --apply`, and archive updates to `.comet/state-events.jsonl`, so workflow progression remains YAML-readable while gaining an auditable event trail.
- **Unified diagnostics**: `comet status` and `comet doctor` now share the same runtime evidence path, exposing the current step, runtime mode, malformed state, missing evidence, and the next recovery action — instead of only listing active changes or install files.
- **`/comet-tweak` path**: `/comet-tweak` is now a tweak-only OpenSpec action chain that runs `openspec-apply-change` during build and supports single-change delta-spec work, while full `/comet` stays on the Superpowers design/plan/build path.
- **Progressive loading for the main comet entry**: The script-location bootstrap (env-var resolution, `--apply`/`transition`/`next`/`archive` command reference, output formats) moves out of the main `comet/SKILL.md` into a new `comet/reference/scripts.md` single source of truth, leaving the entry with a compact key-entry summary and a one-time bootstrap reference. The main entry shrinks by ~45 lines of inline boilerplate that every entry-point load previously paid for; sub-skills still carry the full bootstrap inline so each remains independently loadable. `assets/manifest.json` registers the new reference file.
- **Comet sub-skill progressive loading**: Extends the bootstrap refactor from /comet to the phase and preset Skills. Frontmatter descriptions now only describe trigger conditions, shared script-location setup lives in comet/reference/scripts.md, and comet/reference/context-recovery.md defines one any-entry recovery rule so resuming from /comet-open, /comet-build, /comet-hotfix, /comet-tweak, or the other Comet entry points stays deterministic without repeating the same boilerplate in every Skill.
- **Repository layout**: Source code is reorganized into `app/`, `domains/`, `platform/`, and `scripts/` so CLI commands, domain logic, platform integration, and build tooling are easier to maintain. Packaged CLI and installed Skill assets are unchanged.
- **Protocol-driven `/comet-any` authoring**: Creating a Skill now runs through an explicit authoring protocol (`reference/authoring-protocol.json`) with a deterministic lane DAG, schema-validated lane outputs, and a real multi-vote skill review. LLM-authored content (entry/node Skill bodies, decision points, recovery) is merged into the generated package via `comet creator authoring-record`, while the deterministic backbone (workflow protocol, runtime scripts, manifest) stays templated and reproducible — so generated Skills carry real authored content on top of a stable scaffold instead of being thin deterministic shells. New `comet creator authoring-plan` and `comet creator authoring-record` commands drive the pipeline, and recorded review evidence (no longer a hardcoded approval) gates publish readiness.
- **Authored content zones (Auto + Authored split)**: Generated Skills no longer try to template domain decisions. Each generated SKILL.md is composed from a deterministic **Auto zone** (frontmatter, route table, Entry/Exit checks, evidence format, recovery — the invariant control plane) plus an **Authored zone** (`## Decision Core` for the entry, `## Guidance` for nodes) that the skill-core / workflow-entry subagents write dynamically. Nodes are classified `delegates` (overlay → installed rich Skill; thin guidance is correct) vs `substance` (workflow-kernel; rich authored guidance is mandatory). A `substance` node without authored content renders an explicit `AUTHORING PENDING` stub and is returned in `unauthoredSubstanceNodes`, which blocks publish readiness — so the generator can no longer ship a fake-complete thin Skill. This is what lets a generated Skill reach Comet-hand-authored quality: the agent authors the decisions, the template provides the scaffold.
- **`/comet-any` Skill Creator naming**: Standardizes the user-facing `/comet-any` flow as Skill Creator across CLI help, Skill guidance, docs, readiness output, and generated metadata so users see one creation model while `factory-*` remains internal module terminology hidden behind `comet creator`.
- **Skill Creator user path**: Documentation and CLI help now route ordinary Skill creation through `/comet-any`, `comet eval`, `comet creator status/next`, and `comet publish review/approve/run/distribute`; `comet bundle` is documented as an advanced backend, while `comet skill run/continue` is labeled as advanced Engine Run debugging.
- **Skill Creator eval readiness**: Publish readiness now accepts repository `comet eval` result JSON for the current draft hash and reports failed eval quality gates with their summaries, so generated Skills are gated on the same eval evidence users run from the repository.
- **Claude Code custom agents for generated Skills**: Skill Creator bundles now declare and distribute platform-native Claude Code custom agent definitions separately from portable role briefs, so Claude project installs receive `.claude/agents/*.md` alongside the generated Skill package.
...
- **Comet eval rubric docs**: Eval README now documents all nine Comet rubric dimensions, weights, workflow-specific scoring behavior, Skill invocation evidence rules, and weighted-score semantics.
...
- **Skill Creator eval identity**: Repository eval results now bind to the current Bundle draft hash and generated eval manifest hash, while generated manifests keep a stable `<current-bundle-hash>` placeholder instead of being rewritten during generation. This prevents stale `comet eval` evidence from clearing publish readiness or overwriting accepted eval evidence without changing the draft hash as a side effect.
- **Repository verification hygiene**: Architecture lint now ignores pytest basetemp directories under `eval/`, preventing local eval test leftovers from being reported as source files outside approved code roots.
...
- **Comet skill doc consistency pass**: Fixes several contradictions and stale descriptions introduced across the `review_mode` redesign rounds, all reconciled in both EN and ZH: (1) the `executing-plans` exit condition no longer unconditionally required loading `requesting-code-review` even under `review_mode: off`, which directly contradicted the review gate; (2) the execution-method recommendation table dropped the obsolete "two-phase review" wording for `subagent-driven-development`; (3) the main `comet/SKILL.md` hard-constraints list now includes `review_mode` alongside `tdd_mode`; (4) the `.comet.yaml` field-reference example now includes the `auto_transition` field it documents; (5) "per-task reviewer" / "每任�?reviewer" is used consistently, replacing the mixed "task-level reviewer" / "任务�?reviewer"; (6) the build-phase review budget table no longer claims a per-`review_mode` verify-phase review column it never implemented �?verify-phase review is correctly attributed to `verify_mode` with `review_mode` only gating whether it fires.
...
- **Eval dual-agent execution on Windows/WSL**: Fixes local eval runs launched from WindowsApps/WSL bash by translating mounted paths correctly, bridging Anthropic proxy environment variables into WSL, passing large prompts through workspace files instead of shell argv, and writing eval logs as UTF-8 so real `auto_user` runs can execute and save results on Windows.
...
- **Classic script split coverage**: Adds regression coverage that shipped Classic scripts no longer import `comet-runtime.mjs`, that the asset manifest omits the removed dispatcher, and that app, engine, benchmark, eval mirror, and `/comet-any` contract paths keep working with independent command scripts.
...
- **Skill Creator eval identity coverage**: Adds regression coverage that generated eval evidence is accepted only for the current Bundle draft hash and eval manifest hash, that stale manifest results cannot replace accepted repository eval evidence, and that the `comet creator` resume surface stays separate from publish and legacy Bundle creator commands.
- **Eval Windows harness coverage**: Adds regression coverage for WSL path conversion, WSL environment bridging, UTF-8 Skill/log handling, and the registered eval task index.
- **Comet eval dependency coverage**: Adds regression coverage that `COMET_FULL_040_BETA` includes Comet, OpenSpec, and Superpowers dependency Skills, that Comet task prompts require real `/comet` invocation, and that missing Comet Skill invocation is a hard eval failure.
- **Comet eval instruction coverage**: Adds regression coverage that Comet workflow evals load the editable benchmark `CLAUDE.md` instruction while generic evals stay unchanged.
...
- **Antigravity 2.0 platform coverage**: Adds environment-isolated detection, global installation, global uninstall, and Skill agent mapping coverage for the new `antigravity2` platform without relying on the host user's real home directory.
...
- **Best-practices section in main comet skill**: Drops the "Best Practices" / "最佳实�? appendix from `comet/SKILL.md`. Every item duplicated more precise guidance in Decision Core or the relevant sub-skill (comet-build / comet-archive / comet-hotfix / comet-tweak), so the section added repetition without agent value; the sub-skills remain the single source of truth.
- **Pre-release eval compatibility formats**: Removes support for the temporary `native-skill-creator` / `comet-fallback` eval result schemas and Bundle eval `benchmark-*` command wording. `comet eval` schema v2 with provider `comet-eval` is now the only accepted evidence format.
- **Pre-release Bundle creator aliases**: Removes the unshipped `comet bundle factory-*`, `comet bundle authoring-*`, `comet bundle list`, and `comet bundle status` CLI entries. Use `comet creator` for creation and resume flows; `comet bundle` now stays focused on draft, compile, eval, review, publish, and distribution backends.

### Security

- **Dependabot dependency alerts**: Pins the development test toolchain to patched Vite and esbuild versions across npm and pnpm lockfiles, clearing open Vite path traversal, launch-editor, and esbuild dev-server advisories without changing Comet runtime dependencies.
