...
- **Comet Any workflow compiler**: Replaces copy-paste call-chain generation with two compiler paths: Customize `/comet` now renders a protected Comet slot overlay, while arbitrary Skill composition renders a Comet-like workflow kernel with routes, gates, pause points, recovery, workflow scripts, and route-conformance eval metadata.
...
- **Comet Skill Maker fidelity**: Stops pasting source Skill bodies and generated audit sections into user-facing Skills. Customized `/comet` outputs now keep Comet-style routing prose, frontmatter descriptions in English, nested Skill invocation wording, and explicit slot evidence gates.
- **Generated runtime assumptions**: Removes copied Comet runtime resources from customized `/comet` outputs and uses the generated workflow protocol plus workflow scripts as the authority for inserted slots, preventing stale `/comet-design -> /comet-build` routes from bypassing custom steps.
- **Generated workflow gates**: Makes `/comet-any` generated Skills write localized stage guidance, require script-backed evidence before phase exits, and print the next generated stage from workflow scripts so composed Skills advance like Comet instead of relying on copied prose.
- **Generated semantic guards**: Makes `/comet-any` generated workflow scripts check machine-readable semantic evidence, source Skill completion markers, and customized `/comet` `.comet.yaml` phase transitions before advancing stages.
- **Arbitrary stage names**: Allows non-Comet `/comet-any` workflows to keep user-provided structured `stageNames` with phase, step, and label metadata instead of rejecting them when no Comet template hint exists.
- **Generated workflow route authority**: Aligns generated Engine manifests, guardrails, eval route conformance, and plan scripts with the compiled workflow protocol so customized `/comet` slots cannot be skipped or replaced by raw source Skill names.
...
- **Factory review artifacts**: Includes `reference/authoring-lanes.json` and `reference/skill-review.md` in generated Factory bundles so review outcomes travel with both Customize `/comet` and arbitrary Skill workflow drafts.
...
- **Generated gate coverage**: Adds regression coverage for localized generated stage prose, workflow evidence recording, script-blocked exits, and script-emitted next-stage routing.
- **Generated route authority coverage**: Adds regression coverage that compares generated Engine YAML, guardrails, eval manifests, and recovery order against the compiled workflow route, including customized `/comet` design and build slots.
...
- **Semantic guard coverage**: Adds regression coverage for arbitrary Skill semantic checks, customized `/comet` `.comet.yaml` phase binding, and structured non-Comet stage-name overrides.
