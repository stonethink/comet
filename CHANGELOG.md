...
- **Run state storage split**: Moves Run engine fields (`run_id`, `skill`, `skill_version`, `skill_hash`, `orchestration`, `current_step`, `iteration`, `pending`, `*_ref`, `run_status`, `run_retries`) from `.comet.yaml` to `.comet/run-state.json`. The yaml now contains only user-facing Classic fields plus a `run_id` link. Machine-owned field protection in `comet-state set` simplifies since Run fields are no longer in the yaml. Legacy changes with Run fields embedded in `.comet.yaml` are auto-migrated on first read.
...
- **Run state migration test alignment**: Adapts 16 test assertions across 8 test files to read Run fields (`current_step`, `skill`, `pending`, `trajectory_ref`, etc.) from `.comet/run-state.json` instead of `.comet.yaml`, matching the new split-state architecture. Removes stale Run field test cases from yaml-level validation, updates error message assertions for unknown-field rejection, and adds backward-compat aliases (`run_status`, `run_retries`) in the benchmark `readState` helper.
