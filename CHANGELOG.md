...
- **Eval scaffold stability**: Eval artifact references and saved report files now keep stable treatment filenames, and local scaffold helper setup no longer depends on an external Git Bash shell just to stage test Skills and `CLAUDE.md` fixtures on Windows.
