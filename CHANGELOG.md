...
- **Factory state initialization backend**: Adds an internal `comet bundle factory-init` path that reads a structured plan file, resolves ordered local Skill preferences, persists Factory metadata into deterministic authoring state, and creates the draft on demand for `/comet-any`. This closes the missing backend half between preference discovery and `factory-generate`, so the user-facing Skill can stay conversational while CLI handles state assembly internally.
- **Factory plan protocol**: Formalizes the `plan.json` contract consumed by `factory-init`, including explicit fields for `preferredSkills`, `callChain`, `deviations`, locale defaults, mode-specific `sourceRoot`, and Engine/runner defaults. This gives `/comet-any` a stable internal handoff shape instead of relying on implicit prompt-only conventions.
...
- **Factory review summaries**: Adds `comet bundle review-summary` as an internal `/comet-any` review handoff that combines Factory metadata, plan evidence, platform compile output, quick/full Eval workload estimates, Eval state, review state, and ready-state evidence before human approval.
- **Factory candidate resolution backend**: Adds `comet bundle factory-resolve` so `/comet-any` can recover from missing or ambiguous preferred Skills through deterministic state updates instead of asking agents to hand-edit authoring JSON. Ambiguous candidates can be resolved to an explicit source root or hash, and ignored missing preferences must record a reason.
- **Resolved Skill evidence artifacts**: Generated Factory Skill packages now include a real Skill evidence summary in `SKILL.md` plus `reference/resolved-skills.json`, recording resolved local Skill sources, descriptions, and hashes so generated workflows are auditable back to real local content.
- **Standalone Skill Runs**: Adds standalone `comet skill run/resume/eval --run-id <id>` support backed by `.comet/runs/<run-id>`, allowing generated Skills to persist Engine state, pending actions, artifacts, snapshots, and runtime Eval evidence even when no OpenSpec change directory exists.
...
- **Manual Skill authoring CLI**: Adds `comet skill install`, `validate`, `inspect`, `run`, `resume`, and `eval` with text and JSON output so advanced users can package, discover, inspect, and drive deterministic Comet Skills without waiting for the agentic authoring layer. Includes discovery precedence, fail-closed overrides, Windows link handling, command JSON contracts, and end-to-end flow tests.
...
- **Snapshot-based manual Runs**: Adds persisted pending actions, immutable snapshot recovery, runtime Eval evidence, artifact merging, and explicit compatible Skill upgrades with migration events while leaving action execution to the current Agent or platform Runtime. Includes multi-step integration tests (plan → implement) with step chaining and completed-run rejection.
- **Bundle distribution service**: Adds ready-state-gated Bundle distribution across selected project/global platforms with locale-aware compilation, explicit optional-capability skips, required-capability cancellation, executable hook confirmation, non-destructive hook settings merges, and per-platform installed/skipped/failed results. Includes tests for hash enforcement, locale overlays, capability gaps, hook preservation, overwrite behavior, and partial platform failure.
...
- **Eval 双基线对比**: Extends the eval harness into a runnable v3-vs-0.3.9 comparison. Snapshots both skill variants (workflow `.mjs` runtime and frozen 0.3.9 `.sh` runtime) under `eval/local/skills/benchmarks/`, adds `COMET_FULL` / `COMET_FULL_039` treatments, and focuses on the full 5-phase `comet-full-workflow` task. Adds an eight-dimension rubric validator (main flow, gate guard, skill invocation, spec drift, completion, efficiency, decision-point compliance, artifact quality) parsed from Claude's stream-json events and workspace artifacts, a `compare_baselines.py` report generator, and an offline `rescore_rubric.py` for re-scoring saved artifacts after heuristic fixes.
- **Eval 多轮交互驱动**: Adds `run-claude-loop.sh` so the agent can complete the full open→archive flow despite Comet's blocking decision points. A user-simulator turn reads the subject agent's pause message and emits an affirmative reply, then resumes the session via `--resume`. Comet-category tasks opt into this loop automatically.
- **Eval Windows/Docker 打通**: Fixes the harness for Windows + Docker Desktop + Anthropic-compatible proxy (BigModel/mimo): resolves git-bash vs WSL `bash` lookup, MSYS path conversion for container-internal `/workspace`, registry-mirror configuration, non-root `agent` user (claude refuses `--dangerously-skip-permissions` as root), `ENV_KEYS` passthrough for `ANTHROPIC_AUTH_TOKEN`/`ANTHROPIC_BASE_URL`/model vars, and `setup.sh` merge semantics for overlapping `.claude/` trees.
- **Eval 可重复与可归因**: Adopts the Harness Eval methodology. Adds `--count N` repetition so reports show mean±stdev and pass-rate distributions instead of a single noisy sample; adds three-bucket failure attribution (workflow/task/model) that classifies every failed check with a reason. Report now shows per-dimension distribution stats and an attribution breakdown.
...
- **Skill-agnostic eval profiles**: Added pytest-compatible eval contracts for arbitrary local Skills, with generic rubric scoring, dynamic Skill paths, configurable interaction loops, generated Skill eval manifests, and a reusable `generic-skill-smoke` task while preserving the existing Comet workflow profile.
...
- **`/comet-any` readiness-gated publish flow**: Strengthens the Skill Factory handoff so generated Engine-enabled Skills also emit `comet/eval.yaml`, review summaries expose explicit `blocked | reviewable | publishable` readiness with plan/eval/review evidence, and ready publish now requires current generated-package proof instead of trusting state alone.
...
- **User-facing diagnostics and readiness output**: Clarifies `/comet-any` text-mode status and review-summary output with explicit readiness, blockers, warnings, and evidence, adds actionable recovery hints plus runtime-eval evidence to `comet status` and `comet doctor`, and aligns the public README/architecture docs with the current Node-only runtime and Skill-platform model.
...
- **Factory-generated Skill synthesis**: Generated Factory Skills now include a composed workflow section and `sourceSummaries` distilled from resolved `SKILL.md` bodies, so the output reflects real Skill behavior rather than only listing candidate names, descriptions, and hashes.
- **`/comet-any` recovery and runner guidance**: Updates the bilingual Skill and references so unresolved candidates route through `factory-resolve`, richer Skill evidence is shown during review, and standalone generated workflows use `.comet/runs/<run-id>` through the internal Skill runner.
...
- **Run state storage split**: Moves Run engine fields (`run_id`, `skill`, `skill_version`, `skill_hash`, `orchestration`, `current_step`, `iteration`, `pending`, `*_ref`, `run_status`, `run_retries`) from `.comet.yaml` to `.comet/run-state.json`. The yaml now contains only user-facing Classic fields plus a `run_id` link. Machine-owned field protection in `comet-state set` simplifies since Run fields are no longer in the yaml. Legacy changes with Run fields embedded in `.comet.yaml` are auto-migrated on first read. Adapts 16 test assertions across 8 test files to the new split-state architecture.
- **Classic compatibility runtime**: Routes state validation, guards, handoff, archive, and hook decisions through one generated TypeScript runtime; the existing shell script names remain as cross-platform facades and no longer contain YAML parsing or transition rules. Internally centralizes change-name validation, active/archive directory resolution, hook governance for parallel changes, restricted `&&` command chains, atomic migration transitions, machine-owned Run projection protection, recoverable handoff/archive transactions, immutable Skill snapshots, fail-closed Run recovery, 5-minute build/verify timeout, lazy guard execution, and phase-specific recovery decomposition. Includes regression tests for archive recovery, change-name rejection, multi-active-change hook blocking, and malformed state rejection.
...
- **Classic diagnostics convergence**: `comet status`, `comet doctor`, and `comet-guard --json` now share one diagnostics path that reports runtime mode, current step, and per-step runtime-eval evidence, moving Classic workflows closer to an Engine-native fact model without changing user entry commands.
...
- **tweak repositioned as an OpenSpec-chained lightweight preset path**: Delta spec becomes a first-class normal artifact instead of an upgrade trigger; when a delta spec is present, tweak explicitly sets `verify_mode full` to run OpenSpec-native verification (`openspec-verify-change`). Upgrade assessment moves to a three-layer division of labor: qualitative-change signals rely on agent semantic recognition, file count is downgraded to a user-decided tripwire, and the scale script only governs `verify_mode` (changedFiles threshold `>4` → `>8`). On an upgrade hit the user chooses between two options (continue preset / upgrade to full). hotfix adopts the same qualitative-signal assessment, drops the "3+ files" hard condition, and fixes the "≤2 files" wording contradiction with the scale threshold. Syncs the comet main SKILL, comet-verify, comet-yaml-fields, and the comet-phase-guard rule (bilingual) plus README ([#121](https://github.com/rpamis/comet/issues/121)).
...
- **Classic malformed-state reporting**: Prevents `comet status` from aborting the whole scan when one change has broken `.comet.yaml`; invalid changes are now reported individually through the shared diagnostics path.
...
- **Init summary duplicate status**: `comet init` no longer shows a platform in both "Installed" and "Failed" sections when some components succeed and others fail. Uses priority-based filtering (failed > installed > skipped) so users see a single, unambiguous status per platform ([#128](https://github.com/rpamis/comet/issues/128)).

### Tests

- **Factory initialization coverage**: Adds command and built-CLI coverage for `factory-init`, including ordered `.comet/skills.txt` preference ingestion, missing-candidate reporting, draft creation, and handoff into the existing factory generation path.
- **Factory readiness coverage**: Adds regression coverage for generated `comet/eval.yaml`, readiness aggregation, review-summary blockers, and factory publish evidence so `/comet-any` publish gates stay aligned with the live authoring contract.
- **Factory plan normalization coverage**: Adds focused tests for default inference, explicit preference index preservation, and optimize-mode `sourceRoot` enforcement in the shared plan-normalization layer.
...
- **Factory review and evidence coverage**: Adds coverage for canonical plan artifacts, unresolved-candidate generation blocking, generated `resolved-skills.json`, review-summary output, and an end-to-end CLI path from plan to review summary with quick/full Eval workload checks.
- **Factory resolve and standalone runner coverage**: Adds command and built-CLI coverage for resolving ambiguous Factory candidates, ignoring missing preferences with reasons, dogfooding the `/comet-any` plan-to-review path through `factory-resolve`, and running deterministic Skills through `.comet/runs/<run-id>`.
- **Classic diagnostics coverage**: Adds focused tests for shared diagnostics, runtime-step eval evidence, malformed-state isolation, generated runtime parity, and JSON guard output so Classic user entry points read from the same fact source.
...
- **Eval report output coverage**: Adds scaffold tests for config-driven HTML report generation and Markdown suppression across local eval summaries and baseline comparison reports.
