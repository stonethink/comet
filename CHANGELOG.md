...
- **`comet publish` command** (new): The user-facing path for `/comet-any` outputs. It reports readiness status, requires review approval, runs the publish step, and previews distribution before writing files to target platforms.
- **`comet publish next` command** (new): Prints one user-facing next step for a Skill Creator candidate without exposing backend Bundle commands, so interrupted `/comet-any` outputs can resume through the publish facade.
...
- **Protocol-driven `/comet-any` authoring**: Creating a Skill now runs through an explicit authoring protocol (`reference/authoring-protocol.json`) with a deterministic lane DAG, schema-validated lane outputs, and a real multi-vote skill review. LLM-authored content (entry/node Skill bodies, decision points, recovery) is merged into the generated package via `comet bundle authoring-record`, while the deterministic backbone (workflow protocol, runtime scripts, manifest) stays templated and reproducible — so generated Skills carry real authored content on top of a stable scaffold instead of being thin deterministic shells. New `comet bundle authoring-plan` and `comet bundle authoring-record` commands drive the pipeline, and recorded review evidence (no longer a hardcoded approval) gates publish readiness.
...
- **`/comet-any` Skill Creator naming**: Standardizes the user-facing `/comet-any` flow as Skill Creator across CLI help, Skill guidance, docs, readiness output, and generated metadata so users see one creation model while `factory-*` remains an advanced backend command family.
...
- **Skill Creator eval identity**: Generated `/comet-any` eval manifests now bind repository eval results to both the current draft hash and the current manifest hash, preventing stale `comet eval` evidence from clearing publish readiness or overwriting accepted eval evidence.
...
- **Skill Creator eval identity coverage**: Adds regression coverage that generated eval manifests align with the current Bundle draft hash and stale manifest results cannot replace accepted repository eval evidence.
