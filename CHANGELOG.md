...
- **New platforms**: Adds ZCode and MimoCode support, including project/global Skill installs, OpenCode-style command generation where appropriate, OpenSpec mirroring, and coverage in init/update and distribution paths.
...
- **Comet skill doc consistency pass**: Fixes several contradictions and stale descriptions introduced across the `review_mode` redesign rounds, all reconciled in both EN and ZH: (1) the `executing-plans` exit condition no longer unconditionally required loading `requesting-code-review` even under `review_mode: off`, which directly contradicted the review gate; (2) the execution-method recommendation table dropped the obsolete "two-phase review" wording for `subagent-driven-development`; (3) the main `comet/SKILL.md` hard-constraints list now includes `review_mode` alongside `tdd_mode`; (4) the `.comet.yaml` field-reference example now includes the `auto_transition` field it documents; (5) "per-task reviewer" / "每任务 reviewer" is used consistently, replacing the mixed "task-level reviewer" / "任务级 reviewer"; (6) the build-phase review budget table no longer claims a per-`review_mode` verify-phase review column it never implemented — verify-phase review is correctly attributed to `verify_mode` with `review_mode` only gating whether it fires.
- **Skill install copy-failure visibility**: `comet init`/`update` no longer swallows individual skill/script copy errors. `copyCometSkillsForPlatform` now tracks a `failed` count, and the per-platform summary marks the platform `failed` with the count (e.g. `failed (12 files, 1 failed)`) instead of reporting a half-installed setup as success — so a missing `comet-hook-guard.mjs` that would silently break phase guard is surfaced.
- **Phase-write hook fails closed on crash**: `comet-hook-guard.mjs` now wraps its top-level dispatch in a try/catch; if it crashes outside `main()` (corrupt bundle, import failure, unexpected throw) it prints a `[COMET-HOOK] crash:` diagnostic and exits 2 (block) rather than leaving the exit code ambiguous, so a broken hook becomes visible instead of silently allowing writes that phase guard should vet.
- **Honest `/comet-any` review evidence + command references**: The factory generator no longer writes a fabricated `Result: approved` review or a hardcoded `review.passed: true`; without a recorded LLM review it emits an explicit `deterministic-check-only` placeholder, and with one it renders the real verdict (voters, lenses, findings). The `/comet-any` Skill no longer references the non-existent `find-skill` command (now `comet bundle candidates` / `comet skill show`), and the documented generated-package script list is aligned with the six scripts the generator actually emits.
