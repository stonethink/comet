...
- Shape: advance after the brief and proposed specifications pass. Sequential mode must obtain the final shared-understanding confirmation and pass `--confirmed`; Batch preserves its existing confirmation rule and passes `--confirmed` only when this turn contains a decision the user just confirmed. On successful entry to Build, the Runtime binds approval to the current contract hash.
