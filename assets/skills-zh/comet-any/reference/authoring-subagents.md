...
在用户确认 Skill Creator 方案后、运行 `factory-generate` 或生成源码前，主会话按以下顺序读取并派发：
...
上面的角色 brief 顺序只是创作 DAG 的线性展开，并非要求严格串行。权威 DAG 在 `reference/authoring-protocol.json` 与 `comet bundle authoring-plan <name> --depth quick|full --json`：
...
- 每个 lane 产出在下一个依赖 wave 开始前，必须经 `comet bundle authoring-record <name> --lane <id> --file <out.json> --json` 校验并记录。
