...
- internal stage Skill：多阶段工作流的每个阶段都应生成一个 internal Skill；entry Skill 只做入口、恢复和总控说明。
- `stageNames`：记录用户确认的阶段命名。proposal 必须展示推荐阶段名和可输入名字项，允许用户为多个阶段输入自定义 internal stage Skill 名称。
...
- `stageNames`：可选。每项指定来源 `skill`、最终 internal stage Skill `name`、可选 `phase` / `step` / `label`。未填写时后端会生成推荐阶段名；填写时必须使用唯一的 kebab-case 名称。
...
- `skillMakerSummary.stageNames`：推荐阶段名、用户自定义名、来源 Skill 和阶段标签；用于渲染可输入名字项。
