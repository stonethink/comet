# Benchmark Provider 参考

## Provider 优先级

优先使用原生 `skill-creator` 的 benchmark/生成能力。原生能力不可用时，必须先询问用户是否允许 Comet fallback；回退前必须询问用户，不得默认启用。
...
- `skip / quick / full benchmark` 三个选择。

skip 或 benchmark 失败时不得进入 ready；也不得 publish 或 distribute。

普通用户的 benchmark 路径保持单一：日常评估统一走 `comet eval`。`/comet-any` 可以在内部调用 `comet bundle benchmark-plan` 和 `comet bundle benchmark-record` 规划与登记证据，但不要把它们包装成面向普通用户的替代 benchmark 命令。
对普通用户，解释时优先使用“验证”，不要把 `Publish readiness:` 当成第一层概念。

## 结果记录

Benchmark provider 需要输出结构化 JSON，随后使用：
...
- 能力缺口和可执行披露。
- benchmark 选择、token 消耗和结果摘要。
- `Validate this Skill` 与下一步提示，让用户知道 readiness 为什么可发布或被阻塞。

readiness blockers 会阻止 publish。只要存在当前 hash 缺失 benchmark 证据、人工 approval 缺失、required capability gap 或 executable disclosure 未确认，就必须停在评审阶段，不能继续发布。
