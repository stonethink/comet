---
name: comet-tweak
description: "Comet 预设路径：串联 OpenSpec 的轻量流程（tweak）。跳过 brainstorming 和完整 plan，直接 open → build → verify → archive。delta spec 作为一等公民正常产物。适用于中等改动、配置调整、文档或 prompt 优化，以及需 spec 驱动但不需架构设计的小变更。"
---

# Comet 预设路径：Tweak

Tweak 是 Comet 五阶段能力的预设工作流，不是独立的平行流程。它串联 OpenSpec 的核心流程，复用 open、build、verify、archive 能力，仅跳过 brainstorming 和完整 plan。

适用于串联 OpenSpec 的轻量改动，例如配置调整、文档或 prompt 优化，以及需 spec 驱动（含 delta spec）但不需架构设计的中等变更。delta spec 在 tweak 中是一等公民正常产物，需要 delta spec 本身不构成升级理由。
...
执行链路：open → build → verify → archive。Tweak 为每个阶段提供默认决策：精简开启、直接构建、按规模与 delta spec 判定验证轻重、验证通过后进入归档前最终确认。
