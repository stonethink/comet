...
- 若 `phase: build`，先检查 `build_pause`、`plan`、`build_mode` 和 `isolation`（详见下方）：
  - 若 `build_pause: plan-ready` 但 `isolation` 和 `build_mode` 已经设置，则视为 stale pause：先输出 `[COMET] 检测到 stale pause（build_pause=plan-ready 但 isolation/build_mode 已设置），自动清除并继续`，再运行 `node "$COMET_STATE" set <name> build_pause null`，然后读取 tasks.md 的下一个未勾选任务并按 `build_mode` 恢复执行
...
1. **质变信号**（agent 语义识别，命中任一即暂停交用户二选一）：跨模块协调修改、需要新增 capability、数据库 schema 变更、引入新的 public API、触及深层架构问题
2. **文件数 tripwire**（用户拍板，非自动升级）：改动文件数超提示阈值时，暂停交用户决定继续 preset 还是升级 full，不自动踢
3. **验证级别**（scale 脚本判定）：`comet-state scale` 仅决定 `verify_mode`（验证轻重），不卡流程、不触发升级

**升级决策点（用户二选一）**：
- 继续 preset 轻量流程（用户确认范围可控）
...
7. 遇到升级判定信号（hotfix/tweak → 用户二选一：继续 preset / 升级完整流程）
...
/comet-hotfix（预设路径，跳过 brainstorming）
  open ──→ build ──→ verify ──→ archive
    ↑ 命中升级判定信号 → 用户二选一（继续 preset / 升级 full）→ 升级则 transition preset-escalate → 补 Design Doc → 回到完整流程
...
11. **Preset 升级判定** — hotfix/tweak 命中质变信号或文件数 tripwire 时，暂停交用户二选一（继续 preset / 升级 full），升级用 `transition preset-escalate`
