...
- `route` 是 runtime 输出或复核后的路由结果，包含下一 Skill、置信度、是否需要用户确认和 fallback 原因。
...
6. `intent.confidence` 或 `route.confidence` 低于阈值，或关键槽位缺少 evidence 时，输出 `ask_user`。
...
assets/skills-zh/comet/scripts/comet-intent.mjs
```

launcher 继续保持薄封装，只 import 生成后的 `comet-runtime.mjs` 并调用 intent 命令。业务逻辑保留在 `domains/comet-classic/`。
...
3. 调用 `node "$COMET_INTENT" route <frame-json>` 或等价命令。
4. 根据 runtime route 进入 `/comet-hotfix`、`/comet-tweak`、`/comet-open` 或用户确认点。

Skill 文档保留人类可读规则，但规则作为填槽指南，而不是最终事实源。
...
- [ ] `/comet` 入口不再把 hotfix/tweak 预设检测只写成自然语言规则，而是要求填充并校验 `CometIntentFrame`。
- [ ] intent runtime 对同一 frame 输出确定性 route。
- [ ] 低置信度、证据不足和冲突场景不会自动选择 workflow。
- [ ] hotfix/tweak 阶段 Skill 不重复入口识别，只复核升级信号。
- [ ] Classic resolver 的阶段路由行为保持不变。
- [ ] 中英文 Skill 在确认后同步。
- [ ] Classic runtime 生成物、manifest 和脚本测试全部同步。
- [ ] Changelog 只描述最终用户可感知的入口路由升级，不记录开发过程。
