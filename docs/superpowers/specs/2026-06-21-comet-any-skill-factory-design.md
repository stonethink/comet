...
- 新增 `src/skill/find.ts` 或扩展 `src/skill/discovery.ts`。
- 支持多根目录、插件 Skill、显式路径、歧义报告、hash、reference 摘要。
- 替换 `src/bundle/candidates.ts` 中只按平台目录扫描的窄实现。

### 13.2 Factory State

- 在 Bundle authoring state 中增加 factory 语义：
  - `factoryGoal`
  - `preferredSkills`
  - `resolvedSkills`
...
- 新增从候选 Skill 和用户目标生成 `comet/skill.yaml` 草稿的适配层。
- 校验生成的 Skill Package 能被 `comet skill validate` 加载。
- 生成 guardrails 和 evals 的最小安全默认值。

### 13.4 Runner

- 支持 `.comet/runs/<run-id>` 或通用 `--run-dir`。
- 提供平台入口可调用的薄 runner 命令。
