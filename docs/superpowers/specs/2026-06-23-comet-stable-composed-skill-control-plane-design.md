...
1. `/comet-any` 生成的新 Bundle 包含完整 required control plane。
2. 新 Bundle 的 `bundle.yaml` 将 `skills/scripts/rules/hooks/references` 声明为 required。
3. 新 Bundle 使用 `checks.yaml`，不再新写 `evals.yaml`。
4. 旧 `evals.yaml` Skill 在迁移期仍可加载。
5. 组合 Skill 能展开来源 `flow.yaml`，并在环路时给出明确错误路径。
6. 未解决 Choice 会阻止生成最终 ready Bundle。
7. `comet eval` 能验证控制面完整性。
8. `comet publish distribute` 能把 hook descriptor 编译到支持的平台，并在不支持的平台 fail closed。
9. 文档只把 `/comet-any -> eval -> publish -> distribute` 作为普通用户主路径。
10. `comet skill` 和 `comet bundle` 的帮助文案明确为低层调试/高级入口。
